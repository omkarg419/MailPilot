...
export function ThreadView({
  ...
  if (isError || !thread) {
    return (
      <main className="flex min-w-0 flex-1 flex-col items-center justify-center bg-background p-6">
        <Alert variant="destructive" className="max-w-md">
          <HugeiconsIcon icon={AlertCircleIcon} strokeWidth={2} />
          <AlertTitle>Failed to load conversation</AlertTitle>
          <AlertDescription>
            Something went wrong while fetching this thread. Try selecting it
            again or refresh the list.
          </AlertDescription>
        </Alert>
      </main>
    );
